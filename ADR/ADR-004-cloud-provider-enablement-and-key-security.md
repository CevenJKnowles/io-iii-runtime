...
ADR-001 established a control plane with local runtime (Ollama). ADR-002 established explicit routing and limited fallback triggers. ADR-003 established logging/retention defaults. This ADR defines the **security boundary** for enabling cloud providers and handling secrets.
...
Requests/documents labelled `audience: internal` (or internal routes) must not be sent to cloud unless:
...
### Option A — Automatic fallback from local to cloud
Rejected:
- violates explicit routing,
- increases accidental disclosure risk,
- makes cost and privacy unpredictable.

### Option B — Keys stored in config files
Rejected:
- high leakage risk,
- poor portability,
- violates standard secret hygiene.

### Option C — Explicit opt-in cloud enablement with strict boundaries (selected)
Accepted:
