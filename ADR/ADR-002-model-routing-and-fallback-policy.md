...
- **Predictability is a feature** (professional systems favor constraint)
- **Determinism and debuggability** via declarative routing and observable paths
- **Governance boundaries**: runtime governance ≠ cognitive governance
- **Hardware realism**: 7B–9B class models are the default envelope; heavier models must be used sparingly
...
Quality and adjudication belong to **cognitive governance**, not runtime governance.
...
This preserves the architecture principle: runtime enforces routing and isolation; cognition and adjudication remain in IO-III Core.
...
### Trade-offs
- Requires disciplined config management to avoid routing drift.
...
### Option A — Heuristic routing (content-based)
**Rejected.**
Violates "explicit, never inferred" routing and increases drift/debug ambiguity.

### Option B — Application-level routing only (no centralized policy)
**Rejected.**
Collapses architectural layers and reduces traceability; makes fallback ad hoc.

### Option C — Mode-driven routing with deterministic fallback (selected)
**Accepted.**
Matches the declared architecture: explicit mode mapping + controlled fallback.
